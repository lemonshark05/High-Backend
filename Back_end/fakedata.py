...
def main():
    ...
    with app.app_context():
        ...
        for i in range(1, 6):
            blog = Blog(title=fake.random_element(elements=sports_related_sentences), content=fake.sentence(ext_word_list=sports_related_sentences), author_id=fake.random_element(elements=[user.id for user in users]), type = fake.random_element(elements=types))
            Crud.create(blog)
